...
export default function SuperAdminDashboard() {
  ...
  const pieData = useMemo(
    () => [
      { name: "Actifs", value: Math.max(Number(dashboard.kpis[0]?.value ?? 0), 0) },
  ...
  const logs = useMemo(
    () => [
      { title: "Activité globale", detail: "Tendance globale stable sur les 30 derniers jours", tone: "success" },
      { title: "CTD critiques", detail: "2 collectivités avec traitement retardé", tone: "warning" },
      { title: "Sécurité", detail: "Aucune anomalie critique détectée", tone: "info" },
      { title: "Performance", detail: "Temps moyen de traitement sous la norme", tone: "success" },
    ],
    [],
  );
  ...
  const activityContext = useMemo(() => {
    const lines = [
      `Rôle: ${role ?? "SUPER_ADMINISTRATEUR"}`,
      "Vue: globale sur l’application",
      `Période: ${period}`,
      `Note: ${dashboard.note}`,
    ...
    return lines.join("\n");
  }, [chartData, dashboard.insights, dashboard.kpis, dashboard.note, logs, period, role]);

  const analyzeWithAi = async () => {
    try {
      setAnalyzing(true);
      const answer = await aiAssistantApi.reporting(
        "Analyse l’ensemble du système, identifie les risques, causes potentielles et solutions prioritaires pour l’application Simba_CTD. Base-toi sur les KPI globaux, les tendances et les journaux de l’activité. Présente le résultat en sections claires : Diagnostic, Causes potentielles, Possibilités de solutions.",
        activityContext,
      );
      setAnalysis(answer);
    } catch {
  ...
  const diagnosisCards = useMemo(
    () => [
      { title: "Diagnostic", icon: BriefcaseBusiness, text: "La plateforme reste globalement stable et les performances sont conformes aux objectifs de supervision." },
      { title: "Causes potentielles", icon: AlertTriangle, text: "Les risques sont concentrés sur quelques collectivités avec dossiers en attente et traitements retardés." },
      { title: "Possibilités de solutions", icon: CheckCircle2, text: "Renforcer la priorisation, l’alerte de relance et la synthèse de pilotage par collectivité." },
    ],
    [],
  );
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[28px] bg-slate-950 p-6 text-white shadow-sm sm:p-8">
        ...
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-3">
            ...
            <div>
              <p className="text-sm text-slate-300">Vue globale de l’application</p>
              <h1 className="mt-1 text-3xl font-semibold sm:text-4xl">Bonjour {username ?? "super-administrateur"}</h1>
            </div>
            <p className="max-w-2xl text-sm text-slate-300">
              Pilotage de toutes les collectivités, suivi des performances et de la qualité globale de la plateforme.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button size="sm" onClick={() => void analyzeWithAi()} disabled={analyzing} className="gap-2 bg-emerald-500 text-white hover:bg-emerald-400">
        ...
        {dashboard.kpis.length > 0 ? (
          ...
          Array.from({ length: 4 }).map((_, index) => (
            <Card key={index} className="border-0 bg-slate-100 shadow-sm">
              <CardContent className="flex min-h-[130px] items-center justify-center text-sm text-slate-500">Chargement des KPI…</CardContent>
            </Card>
          ))
        )}
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <Card className="border-border/70 shadow-sm">
          <CardHeader className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-lg"><TrendingUp className="h-5 w-5 text-emerald-600" />Vue d’ensemble de l’application</CardTitle>
        ...
        <Card className="border-border/70 shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg"><Globe2 className="h-5 w-5 text-violet-600" />Répartition globale</CardTitle>
          </CardHeader>
          <CardContent className="h-[320px] p-4">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
          ...
          <CardContent className="space-y-3">
            {logs.map((log) => (
              <div key={log.title} className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                <div className={`mt-1 h-2.5 w-2.5 rounded-full ${log.tone === "success" ? "bg-emerald-500" : log.tone === "warning" ? "bg-amber-500" : "bg-sky-500"}`} />
        ...
        <Card className="border-border/70 shadow-sm">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-2xl border border-violet-200 bg-violet-50 p-4 text-sm leading-7 text-slate-700 whitespace-pre-wrap">
              {analysis || "Cliquez sur « Analyser IA » pour obtenir le diagnostic stratégique global, les causes potentielles et les solutions prioritaires sur l’ensemble de l’application."}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-border/70 shadow-sm">
        <CardHeader className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2 text-lg"><TrendingUp className="h-5 w-5 text-emerald-600" />Synthèse stratégique</CardTitle>
          <Badge variant="secondary" className="gap-1"><BadgeCheck className="h-3.5 w-3.5" />Plateforme saine</Badge>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-3">
          {dashboard.insights.length > 0 ? (
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500 md:col-span-3">
              Aucune synthèse stratégique n’est disponible pour l’instant.
            </div>
          )}
        </CardContent>
      </Card>
